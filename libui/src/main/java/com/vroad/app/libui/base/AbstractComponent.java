...
//  }
  ...
  @SuppressWarnings("unchecked")
  @Nullable
  default <T extends AbstractViewModel> T createViewModel(@NonNull Class<? extends AbstractViewModel> viewModelClass) {
    try {
      return (T) new ViewModelProvider(getViewModelStoreOwner()).get(viewModelClass);
    } catch (Exception e) {
      XLog.e(e);
      return null;
    }
  ...
  default <T extends AbstractViewModel> T createViewModel(@NonNull Class<? extends AbstractViewModel> viewModelClass,
    ...
    try {
      //((MutableCreationExtras) extras).set(SavedStateHandleSupport.VIEW_MODEL_STORE_OWNER_KEY, getViewStoreOwner());
      ((MutableCreationExtras) extras).set(ViewModelProvider.AndroidViewModelFactory.APPLICATION_KEY, getAbstractApplication());
      return (T) new ViewModelProvider(getViewModelStoreOwner().getViewModelStore(), viewModelFactoryClass.newInstance(), extras).get(viewModelClass);
    } catch (Exception e) {
      XLog.e(e);
      return null;
    }
